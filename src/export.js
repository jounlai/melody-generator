...
/**
 * 1つの音符・休符で書ける長さ（divisions 単位）と、その長さを置いてよい位置。
 * 付点音符は「素の音価の位置」に置く（付点2分は2分の位置から、など）。
 * 長い順に見て、小節内の位置に合うものを採る。
 */
const UNITS = [
  [16, 16], // 全音符
  [12, 8],  // 付点2分
...
function splitDurations(startInBar, divs) {
  ...
  while (left > 0 && guard++ < 4 * DIVS_PER_BAR) {
    let picked = 1;
    for (const [value, align] of UNITS) {
      if (value <= left && pos % align === 0) {
        picked = value;
        break;
      }
    }
